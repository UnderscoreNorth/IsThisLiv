<script lang='ts'>
    ...
    function checkErrors(){
        ...
        for(const type in rosterLimits){
            for(let pos in rosterLimits[type]){
                ...
                if(current < limits[0]) errorsArr.push(`Need at least ${limits[0] - current} more ${type} ${pos}`);
            }
        }
        if(DMFCMF == 0) errorsArr.push(`Need at least 1 starting DMF/CMF`);
        if(benchMedal == 0) errorsArr.push(`Need at least 1 bench medal`);
        if(counts.starting < 11) errorsArr.push(`Need at least ${(11 - counts.starting)} more starting players`);
    ...
    function checkLimits(player:Player){
        ...
        if(rosterLimits[addMode][player.regPos] !== undefined && roster[addMode][player.regPos] >= rosterLimits[addMode][player.regPos][1]) return false;
        if((typeCount >= 6 && addMode == 'bench')||(typeCount >= 11 && addMode == 'starting')) return false;
        if(player.medal !== '' && roster[addMode][player.medal] >= rosterLimits[addMode][player.medal][1])
            return false;
        if(roster[addMode][data.posOrder[player.regPos]] >= rosterLimits[addMode][data.posOrder[player.regPos]][1])
            return false;
    ...
    async function save(){
        ...
        await api('/ff/saveTeam',sendData).then((r)=>{
            if(r.error){
                saveErrors = 'Error: ' + r.error;
            } else {
                saveErrors = 'Saved';
            }
        }).catch(()=>{saveErrors = 'Error: Something wrong happened'});
        saving = false;
    }
    ...
    <parent>
        <div id='containers'>
            <div>
                    ...
                    {#each types as type}
                        <container>
                            <b
                                ...
                                on:click={()=>{addMode = type}}
                            >{type == 'starting' ? 'Starting' : 'Bench'}{addMode == type ? ' - Selected' : ''}</b><hr>
                            <table>
                                <tr><th>Board</th><th>Pos</th><th>Medal</th><th>Player</th></tr>
                                {#each sortTable($ffStore[type]) as playerID}
                                    <PlayerRow player={data.players[playerID]} classes={'click'} onClick={()=>{removePlayer(type,playerID)}}/>
                                {/each}
                            </table>
                        </container>
                    {/each}
                ...
                Vice <select bind:value={$ffStore.vice}>
            ...
            <container>
                <b>Teams</b><hr/>
                <div>
                    <div class='click' on:click={()=>{toggleFilter('team','All')}} >Select All</div>
                {#each data.teams as team}
                    <div on:click={()=>{toggleFilter('team',team)}} class={((filters.team.size && !filters.team.has(team)) ? 'disabled' : '') + ' click'}><TeamIcon {team}/>/{team}/</div>
                {/each}
                </div>
            </container>
            <container>
                <b>Medals</b><hr/>
                <div>
                    <div class='click' on:click={()=>{toggleFilter('medals','All')}} >Select All</div>
                {#each data.medals as medal}
                    <div on:click={()=>{toggleFilter('medals',medal)}} class={((filters.medals.size && !filters.medals.has(medal)) ? 'disabled' : '') + ' click'}>{medal.length ? medal : 'Regular'}</div>
                {/each}
                </div>
            </container>
            <container>
                <b>Pos</b><hr/>
                <div>
                    <div class='click' on:click={()=>{toggleFilter('pos','All')}} >Select All</div>
                {#each data.pos as pos}
                    <div on:click={()=>{toggleFilter('pos',pos)}} class={((filters.pos.size && !filters.pos.has(pos)) ? 'disabled' : '') + ' click'}>{pos}</div>
                {/each}
                </div>
            </container>
            {/if}
        ...
        <table >
                ...
                {#each checkFiltered(data.players) as player}
                    <PlayerRow {player} classes={checkLimits(player) ? 'click' : 'disabled'} onClick={checkLimits(player) ? ()=>{addPlayer(player) } : ()=>{}} showPt={true}/>
                {/each}
                {/key}
            </table>
        </div>
...
{$ffStore.name}
Starting
{sortTable($ffStore.starting).map((x)=>formatPlayerExportRow(x)).join('\n')}
Bench
{sortTable($ffStore.bench).map((x)=>formatPlayerExportRow(x)).join('\n')}

string: {getString()}
            </textarea><br>
            <button disabled={saving} on:click={()=>{save()}}>Save</button>
            {saveErrors}
        </container>
    </div>
        {/if}
