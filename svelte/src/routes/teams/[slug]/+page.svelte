<script lang="ts">
	...
	let data: {
		...
		roster: {
			...
			footer: string;
			data: any;
			cups: Array<{ year: string; season: string }>;
		};
		latestRoster: {
			cup: {
				cupID: number;
...
{:then data}
	...
	{:else}
		<div style="display:flex;flex-direction:column;height:100%">
			<div>
				{#if data.date}
					<div id="pageModifiedTime">
						Last updated - <Datetime date={data.date} multiline={false} />
					</div>
				{/if}
				<h2 id="header">
					<TeamIcon {team} /> /{team}/ -
			...
			{#if copyTimer > 0}
				<div
					id="copyTimer"
					style:opacity={1 - Math.pow((copyTimerMax - copyTimer) / copyTimerMax, 2)}
				>
