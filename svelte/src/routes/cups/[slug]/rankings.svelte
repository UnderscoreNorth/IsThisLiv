...
Underlined rankings indicate teams that advanced despite ranking in the bottom half of the group
stage or teams that failed to advance despite ranking in the top half of the group stage.
<table>
	<tbody>
		{#if data.cupID > 2}
			<tr>
						...
						class={row.gRanking == 1
							...
							: row.gRanking == 2
								...
								: row.gRanking == 3
									? 'Bronze'
									: ''}
						style:text-decoration={(row.gRanking > 16 && row.oRanking < 17) ||
						(row.gRanking < 17 && row.oRanking > 16)
							? 'underline'
							: ''}
					>
						{row.gRanking}</td
