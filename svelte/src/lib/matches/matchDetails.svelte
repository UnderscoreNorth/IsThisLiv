<script lang="ts">
	...
	let req = Promise.all([
		api('/sql/matchDisplay/' + matchID),
		api('/sql/matchHistory/' + matchID)
	]).then((x) => {
		console.log(x);
	...
	function getScore(data: MatchStat, team: string) {
		...
		return data.events[index].filter((x) => {
			if (
				(x.player.team == team && [1, 4].includes(x.event.eventType ?? -1)) ||
				(x.player.team !== team && [3].includes(x.event.eventType ?? -1))
			)
				return true;
			return false;
		}).length;
		...
		<div id="matchContainer">
			<div id="matchMeta">
				<table style="margin-left:auto;margin-right:auto;">
					<tbody>
						<tr>
							<th>ID</th><th>Stage</th><th>Date</th><th>Stadium</th><th>Attend</th><th>Winner</th
							><th>Official</th><th>Valid</th>
							<th>Pes</th>
						</tr>
						<tr>
							<td>{matchID}</td>
				...
				<h3>
					...
					<TeamLink team={data.teams[2]} /><TeamIcon team={data.teams[2]} />
				</h3>
			</div>
			<div id="matchStats">
											...
											{#if data.performances[i][j]}
												<tr>
													<td
														...
														style={data.motm == data.performances[i][j].player.playerID
													...
													>
														<a
															class="playerLink"
															title={data.players[i].filter(
																(x) => x.player.playerID == data.performances[i][j].player.playerID
															)[0].player.name}
															href="/players/{data.players[i].filter(
																(x) => x.player.playerID == data.performances[i][j].player.playerID
															)[0].player.linkID}-{data.players[i]
																.filter(
																	(x) =>
																		x.player.playerID == data.performances[i][j].player.playerID
																)[0]
																.player.name?.replaceAll(' ', '')}"
															>{data.players[i].filter(
																(x) => x.player.playerID == data.performances[i][j].player.playerID
															)[0].player.name}</a
														>
													</td>
													<td
														><b><CondIcon cond={data.performances[i][j].performance.cond} /></b></td
													>
													<td>
														{data.performances[i][j].performance.rating}
													</td>
													<td>
														{(data.performances[i][j].performance.saves ?? -1) >= 0
															? data.performances[i][j].performance.saves
															: ''}
													</td>
													<td>
							...
							<table style:margin="auto">
								<tbody>
									{#each events as { event, player }, j}
										<tr>
											<td>
												<a href="/players/{player.linkID}-{player.name?.replaceAll(' ', '')}"
													>{player.name}</a
												>
											</td>
											<td>
							...
							<div style:width={'50%'}>
								<table style:margin="auto">
									{#each penalties as { penalty, player }, j}
										<tr>
											<td>
												{data.players[i].filter((x) => x.player.playerID == player.playerID)[0]
													.player.name}
											</td>
						...
						<tbody>
							...
							<tr>
								<td colspan="3"
									>{history.matches.filter(
										(x) => x.match.homeTeam == x.match.winningTeam && x.match.valid
									).length}-{history.matches.filter(
										(x) => 'draw' == x.match.winningTeam && x.match.valid
									).length}-{history.matches.filter(
										(x) => x.match.awayTeam == x.match.winningTeam && x.match.valid
									).length}</td
								>
							...
							{#each history.matches as { match, cup }}
								<tr style:text-decoration={match.valid ? '' : 'line-through'}>
									<td class={getResult(match.homeTeam, match.winningTeam)}
										><TeamIcon team={match.homeTeam} /> /{match.homeTeam}/</td
									>
									<td
										>{#if match.endPeriod > 1}
											{['', '', 'A.E.T', 'Pens'][match.endPeriod]}<br />
										{/if}
										{match.homeGoals} - {match.awayGoals}</td
									>
									<td class={getResult(match.awayTeam, match.winningTeam)}
										>/{match.awayTeam}/ <TeamIcon team={match.awayTeam} /></td
									>
									<td
										><a href="/cups/{cup.cupID}-{cup.cupName.replaceAll(' ', '-')}">{cup.cupName}</a
										></td
									>
									<td>{match.round}</td>
