...
<awayInfo><TeamIcon team={match.away} /><TeamLink team={match.away} /><br /> </awayInfo>
<rightInfo>
	{#if $User.user}
		...
		<tbody>
			{#each match.homeE as e}
				<tr>
					<td
						><i
							>{#if e.secondary !== undefined}<a
									href="/players/{e.secondary.p.linkID}-{e.secondary.p.name}"
									>{e.secondary.p.name}</a
								>
								<EventIcon eventType={e.secondary.e.eventType} />{/if}</i
						></td
					>
					<td><a href="/players/{e.primary.p.linkID}-{e.primary.p.name}">{e.primary.p.name}</a></td>
					<td
						>{e.primary.e.regTime}{e.primary.e.injTime >= 0 ? '+' + e.primary.e.injTime + "'" : "'"}
						<EventIcon eventType={e.primary.e.eventType} /></td
					>
				</tr>
			{/each}
		</tbody>
	...
	<table style:margin-right="auto">
		<tbody>
			{#each match.awayE as e}
				<tr>
					<td
						><EventIcon eventType={e.primary.e.eventType} />
						{e.primary.e.regTime}{e.primary.e.injTime >= 0
							? '+' + e.primary.e.injTime + "'"
							: "'"}</td
					>
					<td><a href="/players/{e.primary.p.linkID}-{e.primary.p.name}">{e.primary.p.name}</a></td>
					<td
						><i
							>{#if e.secondary !== undefined}<EventIcon eventType={e.secondary.e.eventType} />
								<a href="/players/{e.secondary.p.linkID}-{e.secondary.p.name}"
									>{e.secondary.p.name}</a
								>{/if}</i
						></td
					>
...
{#if match.penalties[0].length}
	<scoreInfo style:grid-column={'3'}> Penalties </scoreInfo>
	<homeInfo style:grid-column={'span 2'} style:grid-row={'4'}>
		<small>
			{#each match.penalties[0] as p}
				<a href="/players/{p.player.linkID}-{p.player.name}">{p.player.name}</a>
				<PenaltyIcon goal={p.goal} /><br />
			{/each}
		</small>
	</homeInfo>
	<scoreInfo style:grid-row={'4'}>
		{match.penalties[0].filter((x) => x.goal).length}-{match.penalties[1].filter((x) => x.goal)
			.length}
	</scoreInfo>
	<awayInfo style:grid-row={'4'}>
		<small>
			{#each match.penalties[1] as p}
				<PenaltyIcon goal={p.goal} />
				<a href="/players/{p.player.linkID}-{p.player.name}">{p.player.name}</a><br />
			{/each}
		</small>
	</awayInfo>
{/if}
