...
class model {
    ...
    static team = async(req, res, next) =>{
        ...
        let sql = await DB.query(`
            ...
            AND sWinningTeam <> '' ORDER BY dUTCTime`,[team,team]);
        let Btemplate = {
            gd:0,
            results:[]
        }
        ...
        for(let i in sql){
            ...
            let subsql = await DB.query(`
                ...
                WHERE iMatchID = ? AND iType IN (1,3,4)`,[team,team,team,team,row.iID]).then(function(result){
                    return result[0];
                });
            let tg = subsql.tg;
            let eg = subsql.eg;
            ...
            if(gd > stats[row.iType].bWin.gd){
                ...
                stats[row.iType].bWin.results.push(`${tg} - ${eg} /${e}/ ${h.dateFormat(row.dUTCTime)}`);
            }
            if(gd < stats[row.iType].bLose.gd){
                ...
                stats[row.iType].bLose.results.push(`${tg} - ${eg} /${e}/ ${h.dateFormat(row.dUTCTime)}`);
            }
        }
