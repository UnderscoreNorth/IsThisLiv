...
class model {
    static main = async(req, res, next) =>{
        ...
        let sql = await DB.query("SELECT PlayerLinkDB.*, COUNT(*) AS 'c', AVG(iCond) AS 'cond', MIN(iSubOn) FROM PlayerLinkDB INNER JOIN PlayerDB ON PlayerLinkDB.iID = PlayerDB.iLink INNER JOIN PerformanceDB ON PlayerDB.iID = PerformanceDB.iPlayerID GROUP BY PlayerDB.iLink HAVING MIN(iSubOn) > 0 AND COUNT(*) > 2 ORDER BY COUNT(*) DESC");
        for(let i in sql){
            let row = sql[i];
            html += `<tr>
                <td>${row.c}</td>
