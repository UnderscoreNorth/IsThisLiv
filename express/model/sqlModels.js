...
import {teamLink,pageExpiry} from '../lib/helper.js';
...
class model {
    static matchDisplay = async(req, res, next) =>{
        ...
        let matchMeta = await DB.query("SELECT * FROM MatchDB WHERE iID=?",[matchID]);
        ...
        data.stadiums = [];
        sql = await DB.query("SELECT DISTINCT(sStadium) FROM MatchDB WHERE iCupID=? ORDER BY sStadium",[data.cupID]);
        for(let i in sql){
            data.stadiums.push(sql[i].sStadium);
        }

        data.matchStats = [[],[],[]];
        sql = await DB.query("SELECT * FROM MatchStatDB WHERE iMatchID=? ORDER BY iHalf,bHome",[matchID]);
        for(let i in sql){
            data.matchStats[sql[i].iHalf-1][sql[i].bHome] = [
                ...
                {sql:'iPassComp',name:'Pass completed (%)',value:sql[i].iPassComp},
                ...
                {sql:'iInterceptions',name:'Interceptions',value:sql[i].iInterceptions},
        ...
        for(let i in teams){
            let team = teams[i];
            sql = await DB.query("SELECT * FROM PlayerDB WHERE iCupID=? AND sTeam=? ORDER BY sName",[data.cupID,team]);
            for(let j in sql){
                data.players[i].push(sql[j]);
            }
            sql = await DB.query("SELECT PerformanceDB.* FROM PerformanceDB INNER JOIN PlayerDB ON PerformanceDB.iPlayerID = PlayerDB.iID WHERE sTeam=? AND iMatchID=? ORDER BY iSubOn,PerformanceDB.iID",[team,matchID]);
            for(let j in sql){
                data.performances[i].push(sql[j]);
            }
            sql = await DB.query("SELECT EventDB.* FROM EventDB INNER JOIN PlayerDB ON EventDB.iPlayerID = PlayerDB.iID WHERE sTeam=? AND iMatchID=? ORDER BY dRegTime,dInjTime,iType",[team,matchID]);
            for(let j in sql){
