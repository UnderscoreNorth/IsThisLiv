...
import { teamLink, pageExpiry, cupLink } from "../../src/lib/helper.js";
...
class model {
  static main = async (req, res, next) => {
    ...
    let sql = await DB.query(
      "SELECT sHomeTeam,iCupID,sRound FROM MatchDB WHERE sROUND LIKE 'Group%' GROUP BY iCupID,sRound,sHomeTeam ORDER BY sRound,MAX(dUTCTime) DESC,sHomeTeam"
    );
    let oldGroup = "";
    let oldCup = "";
    let cups = [];
      ...
      for (let old_row of cups) {
        if (new_row.cup > old_row.cup) {
          ...
          if (sameTeams.length >= 3) {
            ...
            html += `<tr><td>${await cupLink(new_row.cup)}</td><td>${
              new_row.group
            }</td>`;
            for (let team of newTeam) {
              sql = await DB.query(
                `SELECT COUNT(*) AS 'c' FROM MatchDB WHERE iCupID=? AND (sRound = 'Round of 16' OR sRound = 'Round of 32') AND (sHomeTeam=? OR sAwayTeam=?)`,
                [new_row.cup, team, team]
              );
              if (sql[0].c) {
            ...
            html += `</tr><tr><td>${await cupLink(old_row.cup)}</td><td>${
              old_row.group
            }</td>`;
            for (let team of oldTeam) {
              sql = await DB.query(
                `SELECT COUNT(*) AS 'c' FROM MatchDB WHERE iCupID=? AND (sRound = 'Round of 16' OR sRound = 'Round of 32') AND (sHomeTeam=? OR sAwayTeam=?)`,
                [old_row.cup, team, team]
              );
              if (sql[0].c) {
                html += `<td class='status-win'>${teamLink(team)}</td>`;
              } else {
